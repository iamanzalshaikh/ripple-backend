 ...
 * Worker that processes ride event jobs
 */
export const rideEventWorker = new Worker('ride-event-processing', async (job) => {
    logger.info(`[WORKER] Processing job: ${job.name} (ID: ${job.id})`);
    try {
        const io = app.io;
        // ==================== JOB: process-group-ride ====================
        if (job.name === 'process-group-ride') {
            ...
            logger.info(`[process-group-ride] Processing ${participants.length} participants`);
            // Calculate group stats
            const finishedCount = participants.filter((p) => p.finishedRide).length;
            const avgSpeed = participants.reduce((sum, p) => sum + (p.avgSpeed || 0), 0) /
                participants.length || 0;
            const maxSpeed = Math.max(...participants.map((p) => p.maxSpeed || 0), 0);
            logger.info(`[process-group-ride] Stats - Finished: ${finishedCount}, Avg Speed: ${avgSpeed}`);
            // Award group badges
            const badgesAwarded = [];
            // Badge: Perfect Ride (zero SOS)
            const sosCounts = participants.filter((p) => p.sosTriggered).length;
            if (sosCounts === 0) {
                const badge = await Badge.findOne({ code: 'perfect_ride' });
                if (badge) {
                    for (const participant of participants) {
                        ...
                        if (!existingAward) {
                            ...
                            badgesAwarded.push({ userId: participant.userId, badgeCode: 'perfect_ride' });
                            // Update user
                            await User.updateOne({ _id: participant.userId }, { $addToSet: { badges: badge._id }, $inc: { totalBadges: 1 } });
                            logger.info(`[process-group-ride] Badge perfect_ride awarded to ${participant.userId}`);
                        }
                    }
                }
...
export default rideEventQueue;
//# sourceMappingURL=rideEvent.queue.js.map