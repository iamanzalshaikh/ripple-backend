 ...
 */
export const sosWorker = new Worker('sos-alerts', async (job) => {
    logger.info(`[SOS WORKER] 🔄 Processing job: ${job.name} (ID: ${job.id})`);
    try {
        if (job.name === 'send-alerts') {
            ...
            for (const contact of sortedContacts) {
                if (!contact.email) {
                    logger.warn(`[SOS] ⚠️ No email for contact ${contact.name}, skipping`);
                    continue;
                }
                try {
                    ...
                    if (emailSuccess) {
                        ...
                        logger.info(`[SOS] ✅ Email sent successfully to ${contact.email}`);
                    }
                    else {
                        logger.warn(`[SOS] ❌ Email failed for ${contact.email}, will retry`);
                    }
                }
                catch (error) {
                    logger.error(`[SOS] ❌ Error sending email to ${contact.email}: ${error.message}`);
                }
            }
        ...
        if (job.name === 'send-location-update') {
            ...
            for (const contact of sortedContacts) {
                ...
                try {
                    logger.info(`[SOS] 📧 Sending location update email to ${contact.name}...`);
                    const emailSuccess = await sendEmailAlert(contact.email, {
                        contactName: contact.name,
                        location,
                        liveUrl: `${config.FRONTEND_URL}/api/v1/safety/live/${sosLog.liveShareToken}`,
                    ...
                    if (emailSuccess) {
                        logger.info(`[SOS] ✅ Location update email sent to ${contact.email}`);
                    }
                    else {
                        logger.warn(`[SOS] ❌ Location update email failed for ${contact.email}, will retry`);
                    }
                }
                catch (error) {
                    logger.error(`[SOS] ❌ Error sending location update to ${contact.email}: ${error.message}`);
                }
            }
            logger.info(`[SOS] ✅ Location update emails processed for ${contacts.length} contacts`);
            return {
                success: true,
                sosId,
        ...
        logger.error(`[SOS WORKER] ❌ Job ${job.name} failed:`, error.message);
        throw error;
    }
}, {
    connection: bullmqRedis,
...
//# sourceMappingURL=sos.queue.js.map