...
export const addBike = async (req, res) => {
    ...
    try {
        ...
        const { brand, model, year, cc, color, registrationNumber, primary, notes, } = req.body;
        ...
        const bikeCount = await Bike.countDocuments({ userId, status: "active" }, { session });
        if (bikeCount >= 10) {
            await session.abortTransaction();
            res.status(400).json({
                success: false,
        ...
        if (bikeCount === 0 || primary === true) {
            ...
            await Bike.updateMany({ userId, _id: { $ne: newBike._id } }, { primary: false }, { session });
        }
        await newBike.save({ session });
        await User.findByIdAndUpdate(userId, { $push: { bikes: newBike._id } }, { session });
...
export const updateBike = async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        const { brand, model, year, cc, color, registrationNumber, primary, mileage, notes, imageUrl, } = req.body;
        ...
        const updates = {};
        if (brand)
            updates.brand = brand.toUpperCase();
        if (model)
            updates.model = model;
        if (year)
 ...
 */
export const setPrimaryBike = async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        ...
        await Bike.updateMany({ userId, _id: { $ne: id } }, { primary: false });
        // Set this as primary
        const updatedBike = await Bike.findByIdAndUpdate(id, { primary: true }, { new: true });
...
export const deleteBike = async (req, res) => {
    ...
    try {
        ...
        const deletedBike = await Bike.findByIdAndUpdate(id, { status: "archived" }, { new: true, session });
        ...
        const primaryBike = await Bike.findOne({ userId, primary: true, status: "active" }, null, { session });
        if (!primaryBike) {
 ...
 */
export const getPrimaryBike = async (req, res) => {
    try {
        const userId = req.userId;
 ...
 */
export const getBikeCount = async (req, res) => {
    try {
        const userId = req.userId;
