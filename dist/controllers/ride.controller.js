...
export const startRide = async (req, res) => {
    try {
        ...
        if (liveShareToken) {
            await redisClient.setEx(`live:${liveShareToken}`, 86400, // 24 hours
            JSON.stringify({
                rideId: ride._id,
                userId,
                lastLocation: null,
 ...
 */
export const streamChunk = async (req, res) => {
    try {
        ...
        const validChunk = chunk.map((point) => {
        ...
        const pointsJson = validChunk.map((p) => JSON.stringify(p));
        ...
        if (ride.liveShareToken) {
            ...
            await redisClient.setEx(`live:${ride.liveShareToken}`, 86400, JSON.stringify({
                rideId: ride._id,
                userId,
                lastLocation: {
                    lat: lastPoint.lat,
 ...
 * Pause current ride
 */
export const pauseRide = async (req, res) => {
    try {
 ...
 * Resume paused ride
 */
export const resumeRide = async (req, res) => {
    try {
...
export const endRide = async (req, res) => {
    try {
        ...
        // Parse points
        const allPoints = pointsData.map((p) => JSON.parse(p));
        logger.info(`[endRide] Processing ${allPoints.length} GPS points for ride ${rideId}`);
        // ✅ Calculate stats
        const polyline = allPoints.map((p) => ({ lat: p.lat, lng: p.lng }));
        const distanceKm = calculateDistance(polyline);
        const distanceMeters = Math.round(distanceKm * 1000);
        const duration = Math.round((allPoints[allPoints.length - 1].timestamp - allPoints[0].timestamp) / 1000);
        const speeds = allPoints.map((p) => p.speed * 3.6);
        const maxSpeed = Math.max(...speeds);
        ...
        await rideQueue.add('analyze-ride', { rideId: ride._id, userId }, { priority: 1 });
        await rideQueue.add('award-badges', {
            rideId: ride._id,
            userId,
            distanceMeters,
            duration
        ...
        // Cleanup old telemetry after 7 days
        await rideQueue.add('cleanup-telemetry', { rideId: ride._id }, { delay: 7 * 24 * 60 * 60 * 1000 });
        logger.info(`[endRide] Queued background jobs for ride ${rideId}`);
        return res.json({
            success: true,
            data: {
...
export const getRideById = async (req, res) => {
    try {
        ...
        // Privacy check
        if (ride.privacy === 'private' && ride.userId._id.toString() !== req.userId) {
...
export const getLiveRide = async (req, res) => {
    try {
        ...
        return res.json({
            ...
            data: {
                rideId: ride._id,
                riderName: ride.userId.name,
                riderAvatar: ride.userId.avatar,
                bike: `${ride.bikeId.brand} ${ride.bikeId.model}`,
