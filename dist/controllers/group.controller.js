...
export const getGroupDetail = async (req, res) => {
    try {
        ...
        const isAdmin = group.members.some((m) => m.userId._id.toString() === userId && m.role === 'admin');
        const hasRequestPending = group.joinRequests.some((r) => r.userId._id.toString() === userId);
        res.json({
            success: true,
            data: {
                ...group,
...
export const joinGroup = async (req, res) => {
    try {
        ...
        const hasPendingRequest = group.joinRequests.some((r) => r.userId.toString() === userId);
...
export const approveJoinRequest = async (req, res) => {
    try {
        ...
        const isAdmin = group.members.some((m) => m.userId.toString() === adminId && m.role === 'admin');
        if (!isAdmin) {
            res.status(403).json({ success: false, error: 'Only admin can approve' });
            return;
        }
        // Remove from requests
        group.joinRequests = group.joinRequests.filter((r) => r.userId.toString() !== requestUserId);
        // Add to members
        const alreadyMember = group.members.some((m) => m.userId.toString() === requestUserId);
        if (!alreadyMember) {
            group.members.push({
