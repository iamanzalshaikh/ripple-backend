 ...
 */
export const reorderEmergencyContacts = async (req, res) => {
    try {
...
export const updateAvatar = async (req, res) => {
    ...
    const avatarUrl = await uploadOnCloudinary(req.file.buffer, "heridez/avatars");
    if (!avatarUrl) {
        return res.status(500).json({
            success: false,
...
export const updatePrivacySettings = async (req, res) => {
    const user = await User.findByIdAndUpdate(req.userId, { privacySettings: req.body }, { new: true });
    res.json({
        success: true,
