...
export const updateSOSLocation = async (req, res) => {
    try {
        ...
        await sosLog.save();
        logger.info(`[SOS] 📍 Location updated for SOS ${id} - Total updates: ${sosLog.locationHistory.length}`);
        ...
        logger.info(`[SOS] 📍 Location updated - Live tracking page will auto-update (no email sent)`);
        return res.json({
            success: true,
...
export const getLiveSOSTracking = async (req, res) => {
    try {
        ...
        const ride = sosLog.rideId
            ? await Ride.findById(sosLog.rideId).select('distance duration avgSpeed simplifiedPolyline')
            : null;
...
//# sourceMappingURL=soslog.controller.js.map