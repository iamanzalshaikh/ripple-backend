...
export const getRideEventDetail = (req: AuthRequest, res: Response) : void => {
  (async () => {
    ...
    const isParticipant = ride.participants.some((p: any) => p.userId._id.toString() === userId);
...
export const rsvpRideEvent = (req: AuthRequest, res: Response) : void => {
  (async () => {
    ...
    const alreadyJoined = ride.participants.some((p: any) => p.userId.toString() === userId);
    if (alreadyJoined) {
...
export const streamRideEventLocation = (req: AuthRequest, res: Response) : void => {
    ...
    try {
      ...
      const isParticipant = ride.participants.some((p: any) => p.userId.toString() === userId);
      if (!isParticipant) {
...
eventType: (ride as any).eventType || 'group_ride',
  ...
  export const getRideEventSummary = (req: AuthRequest, res: Response) : void  => {
    ...
    try {
      ...
      const finishedCount = participantStats.filter((p: any) => p.finishedRide).length;
      const avgSpeed =
        participantStats.reduce((sum: number, p: any) => sum + (p.avgSpeed || 0), 0) /
          participantStats.length || 0;
      const maxSpeed = Math.max(...participantStats.map((p: any) => p.maxSpeed || 0), 0);
      const totalElevation = participantStats.reduce((sum: number, p: any) => sum + (p.elevation || 0), 0) /
        participantStats.length || 0;
  
      const userStats = participantStats.find((p: any) => p.userId.toString() === userId);
        ...
        data: {
          ...
          groupStats: {
            avgDistance: formatDistance(
              participantStats.reduce((sum: number, p: any) => sum + (p.distance || 0), 0) /
                participantStats.length || 0
            ),
